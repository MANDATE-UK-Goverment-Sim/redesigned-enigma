...
window.ElectionStore = (() => {
  ...
  function makeDefaultState() {
    return {
      ...
      surprise: {
        ...
        autoExitPoll22: false,
        declaredCount: 0,
        remaining: NEWH_DATA.TOTAL_SEATS,
        lastMessage: 'Manual election mode'
      },
      broadcast: {
  ...
  function normalize(raw) {
    ...
    const state = {
      ...
      surprise: { ...base.surprise, ...(raw.surprise || {}) },
  ...
  async function rpc(name, args = {}) {
    if (!cloudClient) {
      if (!cloudConfigured()) throw new Error('Connect Supabase first in Live Sync.');
      const ok = await connectCloud();
      if (!ok || !cloudClient) throw new Error(lastCloudError || 'Could not connect to Supabase.');
    }
